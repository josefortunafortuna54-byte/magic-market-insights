...
// Fallback offline = catálogo inicial (mesmo seed da migration 20260822010000)
...
/**
 * Catálogo da loja server-side (tabela store_products), padrão local-first:
 * 1. AsyncStorage cache → UI instantânea
 * 2. Supabase (active=true, ordenado por sort_order) → actualiza + grava cache
 * 3. Sem rede e sem cache → DEFAULT_STORE_PRODUCTS
 */
export function useStoreProducts() {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(STORE_PRODUCTS_STORAGE_KEY)
      .then((raw) => {
  ...
  useEffect(() => {
    ...
    fetchServer().then((server) => {
      ...
      setProducts(server);
      AsyncStorage.setItem(STORE_PRODUCTS_STORAGE_KEY, JSON.stringify(server)).catch(() => {});
    });
    return () => { cancelled = true; };
  }, [fetchServer]);
  ...
  const refresh = useCallback(async () => {
    ...
    listRef.current = server;
    setProducts(server);
    await AsyncStorage.setItem(STORE_PRODUCTS_STORAGE_KEY, JSON.stringify(server)).catch(() => {});
  }, [fetchServer]);
